...
public class VivoSignUtils {
	private static final Logger LOG = LoggerFactory.getLogger(VivoSignUtils.class.getName());
	//vivo分配的商户私钥
	...
	public static String getVivoSign(Map<String,String> para, String key) {
		...
		LOG.debug("----------------------------vivo加签拼接，不带key：" + prestr + "----------------------------");
		prestr = prestr + QSTRING_SPLIT + md5Summary(key);
		LOG.debug("----------------------------vivo加签拼接，带key：" + prestr + "----------------------------");
		//11.2修改，urlEncode
		/*try {
			prestr = URLEncoder.encode(prestr,"utf-8");
		}catch (UnsupportedEncodingException e){
			throw new EmaRuntimeException("----------------------vivo URLDecode失败----------------------");
		}*/
